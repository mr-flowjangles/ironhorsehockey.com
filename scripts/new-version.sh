...
if [ "$action" = "create" ]; then
  ...
  #
  # Stub is passed via a temp file (not `awk -v`) because BSD awk on macOS
  # rejects multi-line strings in `-v` assignments with `newline in string`.
  tmp=$(mktemp)
  stub_file=$(mktemp)
  printf '%s\n' "$stub" > "$stub_file"
  awk -v stub_file="$stub_file" '
    BEGIN {
      stub = ""
      while ((getline line < stub_file) > 0) stub = stub line ORS
      close(stub_file)
      inserted = 0
    }
